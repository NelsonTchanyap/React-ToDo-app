...
export default function CreationModal( props ) {
  ...
  useEffect(()=>{
    ...
    creationBox.current.addEventListener("keypress", e => {

      if ( e.key == "Enter" ) {
        panelButtons.current[2].click()
        // alert(e.key)
      }
  ...
  useEffect(()=>{
    document.querySelector(".todo-list").scrollBy( 0 , 9999999999 ) 
    setTodos({ ...Todos , newTodos:[...newTodos]  })
  },[newTodos])
  ...
  const handleCancelation = ( e ) => {
      ...
      toggleModals( { ...modalsState , create : !modalsState.create } )
    ...
    const handleSave = ( e ) => {  
      ...
      if( !requiredMessageStates.includes( true ) && !validationMessageStates.includes( true ) ) {
        ...
        setSavingState( true )
        // creationBox.current.scrollBy( 0 , -170 ) 
        
        panelButtons.current[2].innerHTML = " <span> Saving... </span> "
        e.target.classList.add("onSave")

        panelButtons.current[0].style = " pointer-events : none"
        panelButtons.current[1].style = " pointer-events : none"
        creationBox.current.style = " pointer-events:none"
        ...
        setTimeout(()=>{
          ...
          panelButtons.current[2].innerHTML = " <span> Save </span> "
          panelButtons.current[2].classList.remove("onSave")

          panelButtons.current[0].style = " pointer-events : fill"
          panelButtons.current[1].style = " pointer-events : fill"
          creationBox.current.style = " pointer-events:fill"
          ...
          setTimeout(()=>{
            ...
            toggleModals({...modalsState , create: false})
        ...
        if( inputs.current[0].value.length > 0 && inputs.current[4].value.length  < 7 ) {
          ...
          newTodos.push({ id : new Date().getTime() , added:true, image : URL.createObjectURL( inputs.current[0].files[0]) , 
        ...
        if( inputs.current[0].value.length == 0 && inputs.current[4].value.length  < 7 ) {
          ...
          newTodos.push({ id : new Date().getTime() ,added:true, image : null , 
          designation : inputs.current[1].value , time : `${ inputs.current[2].value } : ${ inputs.current[3].value}`,
          timer : currentTime , selection:{state :false, timer:null}})
        ...
        if( inputs.current[0].value.length > 0 && inputs.current[4].value.length  > 7 ) {
          ...
          newTodos.push({ id : new Date().getTime() ,added:true, image : inputs.current[0].value , 
        ...
        if( streak.length > 0 ){
          streak.push(newTodos[0])
          console.dir(streak)
          setStreak([...streak])
          document.querySelector(".todo-list").scrollBy( 0 , 9999999999 ) 
          setTodos({ ...Todos , newTodos:[...streak]  })
          // setNewTodos([...streak])
        }
  ...
  const handleStreak = ( e ) => {  
    ...
    if( !requiredMessageStates.includes( true ) && !validationMessageStates.includes( true ) ) {
      ...
      panelButtons.current[1].style = " pointer-events : none"
      panelButtons.current[2].style = " pointer-events : none"
      creationBox.current.style= " pointer-events : none"
      
      setTimeout(()=>{

        creationBox.current.style= " pointer-events : fill"
        setStreakState( false )

        if( streakCount === 5 ) {
          
          document.querySelector(".todo-list").scrollBy( 0 , 9999999999 ) 
        }
        panelButtons.current[0].classList.remove("onSave")

        panelButtons.current[1].style = " pointer-events : fill"
        panelButtons.current[2].style = " pointer-events : fill"
      ...
      if( inputs.current[0].value.length > 0 && inputs.current[4].value.length  < 7 ) {
        ...
        streak.push({ id : new Date().getTime() , added:true, image : URL.createObjectURL( inputs.current[0].files[0]) , 
        designation : inputs.current[1].value , time : `${ inputs.current[2].value } : ${ inputs.current[3].value}`,
        timer : currentTime ,selection:{state :false, timer:null} })
      ...
      if( inputs.current[0].value.length == 0 && inputs.current[4].value.length  < 7 ) {
        ...
        streak.push({ id : new Date().getTime() ,added:true, image : null , 
        designation : inputs.current[1].value , time : `${ inputs.current[2].value } : ${ inputs.current[3].value}`,
        timer : currentTime , selection:{state :false, timer:null}})
      ...
      if( inputs.current[0].value.length > 0 && inputs.current[4].value.length  > 7 ) {
        ...
        streak.push({ id : new Date().getTime() ,added:true, image : inputs.current[0].value , 
        designation : inputs.current[1].value , time : null ,
        timer : currentTime , selection:{state :false, timer:null}})
  ...
  return (
    ...
    <div ref = { creationBox } className={ modalsState.create  ? "Modals active" : "Modals"} >
      ...
      <form onSubmit={handleSubmit} action="">
            ...
            <div className='Description'>
              ...
              <div className="image">
                ...
                <div  className="image-placeholder">
                  { ( inputs.current.length > 2 && inputs.current[0].files[0] ) ? (
                      <div>ezezd</div>
                  ):(
                    <div>Home</div>
                  )}
              ...
              <div className="todo-Designation">
                ...
                <div className={ requiredMessageStates[1] ? "input required"
                 : validationMessageStates[1] ? "input warning" : focus[0] ? "input focus" : "input" } >
                  ...
                  placeholder='what do you planify ?' value={ designation } onChange={ ( e ) =>  handleDesignChange(e.target , 0 ) }  /> 
              ...
              <div className="Time">
                ...
                <div className="time-segment">
                  ...
                  " Hour " } >
                    ...
                    <input ref={addInputs} index = "2" type="number" name="hours" min="00" max="23" defaultValue={"00"} onChange={ ( e ) =>  focusable(e.target , 1 ) } onKeyUp={ (e) => onkeyUpon(e , requiredMessageStates , toggleRequiredMessage )}  arrow="false" />
                  ...
                  " Minutes " } >
                    ...
                    <input ref={addInputs}  index = "3" type="number"  name="minutes" min="00" max="59" defaultValue={"00"} onChange={ ( e ) =>  focusable( e.target , 2 ) } onKeyUp={ (e) => onkeyUpon(e , requiredMessageStates , toggleRequiredMessage )} />
          ...
          <div className="button-panel">
            ...
            <button ref = { addButtons } className={onStreakState ? "continue onSave" : "continue" } onClick={handleStreak}> </button>
            ...
            <div className="cancel-save">
              ...
              <button ref = { addButtons } className={savingState ? "save onSave" : "save" } onClick={handleSave}>
                <span> Save </span>
              </button>
